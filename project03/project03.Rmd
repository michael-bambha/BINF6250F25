...
def _compare_ic_mean(previous_ic: deque, current_ic: deque) -> float:
    """Find the difference between the means of two
    deque structures containing information content

    Args:
        previous_ic (deque): Deque of information content from previous iteration
        current_ic (deque): Deque of information content from current iteration

    Returns:
        float: Absolute value of the mean difference between the two
        deques
    """
    return abs(mean(current_ic) - mean(previous_ic))


def check_ic(
    current_ic: deque, previous_ic: deque, patience: int, break_diff: float
) -> bool:
    """Find whether the information content

    Args:
        current_ic (float): ic from 1 to n
        previous_ic (deque): ic from 0 to n-1
        patience (int): # iterations to 
        break_diff (float): _description_

    Returns:
        bool: _description_
    """
    if _compare_ic_mean(previous_ic, current_ic) < break_diff:
        return True
    return False
